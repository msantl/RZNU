...
def stats(request):
    def get(request):
        ...
        with open("stats.log", "r") as log:
            stats = list()
            for line in log:
                l = line.strip().split('\t')
                stats.append((l[0], l[1]))

        context = { 'stats' : stats }
        return render(request, 'snippy/stats.html', context)
...
def api_stats(request):
    def get(request):
        ...
        for (key, val) in stats:
            data.append({'URL/browser' : key,
                         'times_visited' : val})

        return HttpResponse(json.dumps({'stats' : data}), content_type="application/json")
